    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->url->setText(settings.value("lastUrl","https://www.youtube.com/watch?v=NtXMOJGkous").toString().trimmed().simplified());
    ...
    connect(this,&MainWindow::consoleVisibilityChanged,[=](const bool isVisible){
...
void MainWindow::init_player()
{
    ...
    connect(player, QOverload<QMediaPlayer::Error>::of(&QMediaPlayer::error),
        ...
        if(error_string.contains("forbidden",Qt::CaseInsensitive)){
           ...
           if(forbiddenRetryCount>2){//prevent the loop if media is actaully forbidden
               showError(error_string);
           }else{
               settingsWidget->clearEngineCache();
               showError(error_string+"\nThis issue is caused with youtube videos due to obsolete engine cache.\nAuto clearing engine cache and trying again...");
               ui->start->click();
           }
        }else{
            showError(error_string);
...
//read
bool MainWindow::consoleHidden_() const
{
...
void MainWindow::on_start_clicked()
{
    ...
    if(engineProcess!=nullptr){
            ...
            QApplication::restoreOverrideCursor();
        #endif
        consoleUi.textBrowser->append("<br><i style='color:red'>Cancelling current process...</i><br>\n");
        engineProcess->blockSignals(true);
        engineProcess->close();
        engineProcess->disconnect();
        engineProcess->deleteLater();
        ...
        consoleUi.textBrowser->append("<br><i style='color:red'>Process cancelled.</i><br>\n");
    ...
    connect(engineProcess, static_cast<void (QProcess::*)
            (int,QProcess::ExitStatus)>(&QProcess::finished),
            [this]
            (int exitCode,QProcess::ExitStatus exitStatus) {
        if(exitCode==0){
            ...
            if(output.contains("http",Qt::CaseInsensitive)){
                consoleUi.textBrowser->append("<br><i style='color:lightgreen'>Media probe finished.</i><br>\n");
                consoleUi.textBrowser->append("<i style='color:lightgreen'>Loading media...</i>\n");
                ...
                if(output.contains(rx)){
                    QString name = rx.capturedTexts().first().trimmed()+output.split(rx).last().trimmed().simplified();
    ...
    if(engineProcess->waitForStarted(1000)){
        ...
        consoleUi.textBrowser->clear();
        showConsole();
        consoleUi.textBrowser->append("<i style='color:lightgreen'>Started media probe...</i>\n");
    }else{
        consoleUi.textBrowser->clear();
        consoleUi.textBrowser->append("<i style='color:red'>Failed to start media probe.</i>\n");
    }
}
...
void MainWindow::on_changeLocationButton_clicked()
{
    ...
    if(dir.isWritable()){
        ...
        showError("Destination directory is not writable.<br>Please choose another.");
    }
}
...
void MainWindow::on_clip_clicked()
{
    ...
    {
        args<<"-c"<<"ffmpeg -async 1 -ss "+ui->startDur->text().trimmed()+
              " -i \""+player->media().canonicalUrl().toString()+"\" -t "
            ...
            (int exitCode,QProcess::ExitStatus exitStatus)
    {
        ...
        if(exitCode==0)
        {
            ...
            consoleUi.textBrowser->append("<br><a style='color:skyblue' href='"+path+"'>Open file location</a> "
...
{
    if(clipOptionChecker()) return;
    QString path = settings.value("localPath",
                   QStandardPaths::writableLocation(QStandardPaths::MoviesLocation))
                   .toString();

    QString mediaFile = QFileDialog::getOpenFileName(this, tr("Select media file"), path,tr("Media files (*.*)"));
...
void MainWindow::on_screenshotLowerFrame_clicked()
{
    this->takeScreenshot();

}

void MainWindow::on_screenshotUpperFrame_clicked()
{
    this->takeScreenshot();
}
...
void MainWindow::on_youtube_clicked()
{
    if(youtubeWidget== nullptr){
        ...
        youtubeWidget->setWindowTitle(QApplication::applicationName()+" | YouTube Search");
