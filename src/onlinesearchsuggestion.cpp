...
const QString suggestUrl(QStringLiteral(
    "http://suggestqueries.google.com/complete/"
    "search?ds=yt&client=youtube&hjson=t&cp=1&format=5&alt=json&q=%1"));

onlineSearchSuggestion::onlineSearchSuggestion(QLineEdit *parent)
    : QObject(parent), editor(parent) {
  //    blacklist<<"whatsapp status"<<"full
  //    movie"<<"download"<<"movie"<<"movies"<<"lyrics"<<"lyric";
  ...
  connect(popup, SIGNAL(itemClicked(QTreeWidgetItem *, int)),
          SLOT(doneCompletion()));
  connect(editor, &QLineEdit::returnPressed, [=]() { preventSuggest(); });
...
void onlineSearchSuggestion::preventSuggest() { timer.stop(); }
