...
Screenshot::Screenshot(QWidget *parent, QString image_location)
    : QWidget(parent), ui(new Ui::Screenshot) {
  ui->setupUi(this);
  ...
  if (pix.isNull() == false) {
    ...
    ui->screenshotView->setPixmap(pix.scaled(ui->screenshotView->minimumSize(),
                                             Qt::KeepAspectRatio,
  ...
  filenameComplete =
      ui->filename->text().trimmed().simplified() + ui->ext->text();
  ext = ui->ext->text();
}

Screenshot::~Screenshot() { delete ui; }
...
void Screenshot::on_save_clicked() {
  QString path =
      settings
          .value("sc_location", QStandardPaths::writableLocation(
                                    QStandardPaths::PicturesLocation))
          .toString();

  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save Image File"), path + "/" + filenameComplete,
      QString("ImageFile (*" + ext + ")"));
  if (!fileName.isEmpty()) {
    settings.setValue("sc_location", QFileInfo(fileName).path());
    QPixmap pix(image_location);
    if (pix.save(fileName, QString(ui->ext->text().split(".").last())
                               .toUpper()
                               .toUpper()
                               .toLocal8Bit()
                               .data())) {
