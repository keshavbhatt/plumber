    ...
    ui(new Ui::SearchProvider)
{
    ...
    _loader->setColor(QColor("#1e90ff"));
     ui->term->addAction(QIcon(":/icons/youtube-line.png"),QLineEdit::LeadingPosition);
}
...
void SearchProvider::on_search_clicked()
{
    ...
    QString url = QUrl("http://ktechpit.com/USS/Olivia/manual_youtube_search.php?query="+ui->term->text().trimmed()).toString(QUrl::EncodeSpaces);
...
void SearchProvider::processResult(QString reply){
    ...
    foreach (QString str, list) {
            ...
            //get meta from p
            QString p = str.split(QString("<p style=\"margin-left:13.6em  !important\">"))
                    .last().split(QString("</p>")).first();
            QString p_title = p.split("Duration: ").first().prepend("<b>").append("</b>");
