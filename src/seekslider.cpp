...
void seekSlider::paintEvent(QPaintEvent *ev) {
  ...
  QRect rect(groove_rect.left(), groove_rect.top(),
             static_cast<int>((subControlWidth / 100) * groove_rect.width()),
             groove_rect.height()); // 0.6 * groove_rect.width()
...
void seekSlider::wheelEvent(QWheelEvent *e) { e->ignore(); }
